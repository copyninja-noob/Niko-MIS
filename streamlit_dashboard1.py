...
# Database file for remarks
DB_FILE = os.path.join(os.path.dirname(__file__), "dashboard_remarks.db")

# Initialize database and create tables
def init_database():
    """Initialize SQLite database and create remarks table"""
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_cell_key ON remarks(cell_key)')
...
def load_remarks():
    """Load all remarks from SQLite database"""
    init_database()  # Ensure database exists
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()

        cursor.execute('SELECT cell_key, remark_text FROM remarks')
        rows = cursor.fetchall()

        # Convert to dictionary format compatible with existing code
        remarks_dict = {}
        for cell_key, remark_text in rows:
            remarks_dict[cell_key] = remark_text

        conn.close()
        return {"remarks": remarks_dict}

    except Exception as e:
        st.error(f"Error loading remarks from database: {e}")
        return {"remarks": {}}

# Save remark to database
def save_remark(cell_key, remark_text):
    """Save or update a single remark in the database"""
    init_database()  # Ensure database exists
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        ...
        cursor.execute('''
            INSERT OR REPLACE INTO remarks (cell_key, remark_text, updated_at)
            VALUES (?, ?, CURRENT_TIMESTAMP)
        ''', (cell_key, remark_text))
        ...
        st.error(f"Error saving remark to database: {e}")
        return False

# Delete remark from database
def delete_remark(cell_key):
    """Delete a remark from the database"""
    init_database()  # Ensure database exists
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()

        cursor.execute('DELETE FROM remarks WHERE cell_key = ?', (cell_key,))
        conn.commit()
        conn.close()
        return True

    except Exception as e:
        st.error(f"Error deleting remark from database: {e}")
        return False

# Clear all remarks from database
def clear_all_remarks():
    """Delete all remarks from the database"""
    init_database()  # Ensure database exists
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        ...
        st.error(f"Error clearing remarks from database: {e}")
        return False

# Legacy function for backward compatibility (converts dict to individual saves)
def save_remarks(data):
    """Legacy function - saves all remarks from dictionary to database"""
    try:
        # Clear existing remarks
        clear_all_remarks()

        # Insert all new remarks
        for cell_key, remark_text in data.get('remarks', {}).items():
            if not save_remark(cell_key, remark_text):
                return False

        return True

    except Exception as e:
        st.error(f"Error in legacy save_remarks: {e}")
        return False
        ...
        with col_btn1:
            if st.button("💾 Save Remark", use_container_width=True):
                if remark_text.strip():
                    ...
                    if save_remark(cell_key, remark_text.strip()):
